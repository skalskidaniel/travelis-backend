...
| `scraped_at`           | String  |                                                                                                                                                                                                                                                                                             |
| `updated_at`           | String  |                                                                                                                                                                                                                                                                                             |
| `available`            | Boolean | Set to `true` on ingest. If the coordinator scrape finds that an offer is no longer returned (availability-by-absence), it is marked `available = false`. Note: The daily availability job (`jobs.availability`) directly deletes unavailable offers from the table.                         |

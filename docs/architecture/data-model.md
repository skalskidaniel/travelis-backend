...
| `image_url`            | String  | Link to the first image of the offer, or `null` if not available                                                                                                                                                                                                                                                                                                                                                                         |
...
| `scraped_at`           | String  |                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `updated_at`           | String  |                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `available`            | Boolean | Set to `true` on ingest. If the coordinator scrape finds that an offer is no longer returned (availability-by-absence), it is marked `available = false`. Note: The daily availability job (`jobs.availability`) directly deletes unavailable offers from the table, and then triggers a bulk user re-matching run (`MatchingService.bulk_match_users`) on the affected cells to immediately prune the deleted offers from `UserOffers`. |
...
2. **On-the-fly pruning**: when a user fetches their paginated feed via `/api/v2/offers`, the backend attempts to hydrate the batch of offers from the `Offers` table. If any offer is missing (hydration miss), the corresponding stale `UserOffers` row is immediately pruned from DynamoDB and the user's `feed_version` is incremented.
...
3. **Database-Level Deduplication (Cross-Scrape / Existing Offer in DB)**:
   ...
   - If it exists, the system **merges** the existing and new items instead of performing a blind overwrite:
     ...
     - The top-level offer fields (`price_total`, `price_per_day`, `provider`, `external_offer_id`, `referral_url`) are updated from the selected winner record.

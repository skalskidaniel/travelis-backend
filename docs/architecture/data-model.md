...
Four domain tables (on-demand capacity):
...
| `price_per_person`     | Number  | PLN; `price_total / (adults + children)`                                                                                                                                                                                                                                                                         |
| `price_per_day`        | Number  | PLN; `price_total / duration / (adults + children)`                                                                                                                                                                                                                                                              |
| `attractiveness_score` | Number  | Stage 2 composite (used for sorting)                                                                                                                                                                                                                                                                             |
| `referral_url`         | String  | travellead / provider deep link (appends `utm_source=travellead`, `utm_medium=cps`, `utm_campaign`, `a_cid`, `a_aid`)                                                                                                                                                                                            |
| `room_type`            | String  | Normalized room type used for fingerprinting                                                                                                                                                                                                                                                                     |
| `image_urls`           | List    | Up to 8 image URLs for the offer (provider order preserved). Empty list when none available.                                                                                                                                                                                                                     |
...
| `scraped_at`           | String  |                                                                                                                                                                                                                                                                                                                  |
| `updated_at`           | String  |                                                                                                                                                                                                                                                                                                                  |
| `available`            | Boolean | Set to `true` on ingest. When sold (availability job or scrape absence), set to `false` with a 14-day TTL rather than hard-deleted. The availability job then triggers `MatchingService.bulk_match_users` on affected cells so non-favorited matches leave the feed; favorited rows stay until Offer TTL expiry. |

| `ttl` | Number | Epoch seconds. While `available`, equals `departure_date` 00:00 UTC. When soft-deleted as sold, set to now + 14 days (DynamoDB TTL). |
...
| `matched_at`    | String  |                                                                                                        |
| `favorited`     | Boolean | Optional flag (defaults to `false` or absent); `true` if marked as favorite by the user                |
...
**Cascading Deletes & Self-Cleaning**: DynamoDB does not support native cascading deletes when offers are removed from the `Offers` table (due to TTL expiration). Instead, the system relies on:

1. **Eventual consistency**: during the periodic scrape and match run, the match job queries current active offers from `Offers` and syncs them with `UserOffers`. Obsolete non-favorited matches are deleted; favorited rows are retained while the Offer still exists (including soft-deleted sold offers). After Offer TTL expiry, hydration miss pruning removes the leftover `UserOffers` row.
2. **On-the-fly pruning**: when a user fetches their paginated feed via `/v2/offers`, the backend attempts to hydrate the batch of offers from the `Offers` table. If any offer is missing (hydration miss), the corresponding stale `UserOffers` row is immediately pruned from DynamoDB and the user's `feed_version` is incremented. Soft-deleted (`available = false`) offers are excluded from the main feed ZSET but remain visible on `/v2/offers/favorites` until TTL expiry.
...
3. **Database-Level Deduplication (Cross-Scrape / Existing Offer in DB)**:
   ...
   - The top-level offer fields (`price_total`, `price_per_person`, `price_per_day`, `provider`, `external_offer_id`, `referral_url`) are updated from the selected winner record.

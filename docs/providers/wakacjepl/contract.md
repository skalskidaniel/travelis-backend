...
Use `data.availability === true` and `data.status === "OK"`. If the offer is no longer available, the daily availability job soft-deletes it (`available = false`, TTL = now + 14 days); otherwise, the job updates `price_total` (and recomputes `price_per_person` and `price_per_day`) when changed.

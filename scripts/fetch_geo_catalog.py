...
def _fetch_wakacje(client: httpx.Client, path: str) -> list[dict[str, Any]]:
    response = client.get(f"{WAKACJE_BASE_URL}/{path}", headers=WAKACJE_HEADERS, timeout=60)
...
def build_tui_catalog(client: httpx.Client) -> dict[str, Any]:
    response = client.post(TUI_GS_INITIAL_URL, headers=TUI_HEADERS, json=TUI_GS_INITIAL_BODY, timeout=60)
    response.raise_for_status()
    filters = response.json()["filters"]

    region_filter = next(item for item in filters if item.get("filterType") == "REGION")
    airport_filter = next(item for item in filters if item.get("filterType") == "AIRPORT")

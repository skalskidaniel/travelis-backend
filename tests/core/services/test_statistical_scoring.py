...
from core.models.common import BoardType, IataCode, LocationPath, ProviderName
...
def test_small_sample_fallback():
    config = StatisticalScorerConfig(small_sample_threshold=30, small_sample_keep_ratio=0.5)
...
def test_composite_score_ranking():
    config = StatisticalScorerConfig(small_sample_threshold=10, small_sample_keep_ratio=1.0)
    scorer = get_scorer("statistical", config=config)
    ...
    # Offer B has worst price (0.0 norm) but best rating and reviews
    score_b = scores_by_price[Decimal(3000)]
...
def test_performance_large_pool():
    ...
    assert execution_time_seconds < 0.25, f"Scoring 5000 offers took too long: {execution_time_seconds:.4f}s"
